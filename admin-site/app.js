...
const getUserAll = async()=>{
    ...
    let resultado = await ( await fetch("http://localhost/apiRest-master/apiRest-master/apiRest-master/backend/controles/campers.php?op=GetAll",config)).json();//await es para esperar que algo suceda
...
const postUser = async(data)=>{
    ...
    let resPOST = await ( await fetch("https://649afe5dbf7c145d0239d567.mockapi.io/sql",config)).json();
...
const putUser = async(data1)=>{
    ...
    let res = await ( await fetch(`https://649afe5dbf7c145d0239d567.mockapi.io/sql/${idModificar}`,config)).json();
    console.log(res);
    location.reload(true);
}
const deleteUser = async(id) => {
    config.method = "DELETE";
    const response = await fetch(`https://649afe5dbf7c145d0239d567.mockapi.io/sql/${id}`, config);
